...
class S1497 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        findOptimalCombination(0, 0, new boolean[N]);
     ...
     */
    public static void findOptimalCombination(int dep, int guitarsUsed, boolean[] selected) {
        // 모든 기타를 고려한 경우
        if (dep == N) {
        ...
        // 현재 기타를 선택한 경우
        selected[dep] = true;
        findOptimalCombination(dep + 1, guitarsUsed + 1, selected);

        // 현재 기타를 선택하지 않은 경우
        selected[dep] = false;
        findOptimalCombination(dep + 1, guitarsUsed, selected);
    }
}
