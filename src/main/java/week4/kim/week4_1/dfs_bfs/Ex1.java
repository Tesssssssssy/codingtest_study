...
public class Ex1 {
    public static void main(String[] args) {
        ...
        // 11
        System.out.println(ex1.solution(maps));
    }
    ...
    public int solution(int[][] maps) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] dir : dirs) {
                ...
                // 맵 범위를 벗어나거나 벽이 있는 경우 이동하지 않음
                if (nx < 0 || nx >= n || ny < 0 || ny >= m || maps[nx][ny] == 0) {
                    continue;
                }

                // 아직 방문하지 않았거나 더 짧은 거리로 갈 수 있는 경우 거리를 업데이트하고 큐에 추가
                if (dist[nx][ny] == 0 || dist[nx][ny] > dist[cur[0]][cur[1]] + 1) {
        ...
        // 도착 위치까지의 거리가 0이면 도달할 수 없으므로 -1을 반환
        // 그렇지 않으면 해당 거리를 반환
        if (dist[n - 1][m - 1] == 0) {
            return -1;
        } else {
