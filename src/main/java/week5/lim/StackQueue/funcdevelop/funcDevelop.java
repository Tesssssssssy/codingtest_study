package week5.lim.StackQueue.funcdevelop;
...
public class funcDevelop {
    public static int[] solution(int[] progresses, int[] speeds) {
        ...
        // 각 작업이 완료되기까지 필요한 일수를 저장하기 위한 큐

        List<Integer> result = new ArrayList<>();
        // 각 배포 날짜마다 배포되는 기능의 수를 저장하는 리스트
        ...
        while (!days.isEmpty()) {
            int count = 1;
            // 해당 배포일에 배포될 기능의 수를 세는 변수
            int deployDay = days.poll();
            // 큐에서 가장 앞에 있는 요소를 제거(poll())하며, 이는 첫 번째 배포될 기능의 완성일

            while (!days.isEmpty() && days.peek() <= deployDay) {
                // 첫 번째 배포일(deployDay) 이후에 완성되는 기능들 중에서
                // deployDay보다 작거나 같은 완성일을 가진 기능들을 찾는다.

                // 이러한 기능들은 deployDay에 함께 배포될 수 있으므로,
                // count를 증가시키고 큐에서 해당 요소를 제거
            ...
            result.add(count);
            // 계산된 count를 result 리스트에 추가.
            // 이는 deployDay에 배포되는 기능의 수 의미
        }
