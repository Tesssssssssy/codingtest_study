...
public class Solution2 {
    ...
    public static int solution(String[] want, int[] number, String[] discount) {
        ...
        // 슬라이딩 윈도우를 사용하여 10일 간의 할인 품목을 확인
        for (int i = 0; i <= n - 10; i++) {
            // 현재 10일간의 할인 품목과 원하는 품목 비교
            // 가능한 시작일마다 checkMatch를 호출하여, 조건을 만족하는 경우를 센다.
    ...
    /*
        슬라이딩 윈도우: 전체 할인 목록에서 10일 간의 상품을 확인하기 위해 슬라이딩 윈도우 기법을 사용.
        윈도우의 시작 지점을 이동하면서 10일간의 할인 품목이 정현이가 원하는 품목과 일치하는지 검사.
    */

    private static boolean checkMatch(String[] want, int[] number, String[] discount, int start) {
        // 현재 10일간의 할인 품목이 정현이가 원하는 품목과 수량이 일치하는지 확인
